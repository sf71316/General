...
function Datepicker() {
	...
	this.regional[''] = { // Default regional settings
		...
		closeStatus: '關閉視窗', // Status text for close link
		prevText: '&#x3c;上月', // Display text for previous month link
		prevStatus: '顯示上一月日期', // Status text for previous month link
		nextText: '下月&#x3e;', // Display text for next month link
		nextStatus: '顯示下一月日期', // Status text for next month link
...
$.extend(DatepickerInstance.prototype, {
	...
	_generateMonthYearHeader: function(drawMonth, drawYear, minDate, maxDate, selectedDate, secondary) {
		...
		if (secondary || !this._get('changeYear')) {
			...
			year = (minDate ? Math.min(year, minDate.getFullYear()) : year);
